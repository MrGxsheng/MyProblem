...
void solve() {
    ...
    ac.work();
    vector<bool> bad(ac.size());
    for (auto x : pos) {
        bad[x] = true;
    }
    for (int i = 1; i < ac.size(); i++) {
        for (int j = i; j; j = ac.link(j)) {
            if (bad[j]) {
                bad[i] = true;
            }
        }
    ...
    for (int i = 1; i <= 1000; i++) {
        vector<Z> ndp(ac.size());
        for (int j = 1; j < ac.size(); j++) {
            ndp[ac.next(j, 0)] += dp[j];
            ndp[ac.next(j, 1)] += dp[j];
        }
        for (int j = 1; j < ac.size(); j++) {
            if (bad[j]) {
                ndp[j] = 0;
        ...
        dp = ndp;
        f.push_back(accumulate(dp.begin(), dp.end(), Z(0)));
    }
    
    auto q = berlekampMassey(f);
    auto p = (f * q).trunc(q.size() - 1);
    auto ans = linearRecurrence(p, q, N);
    cout << ans << "\n";
}
